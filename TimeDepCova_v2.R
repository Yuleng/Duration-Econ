...
mid <- na.omit(mid); mid <- mid[mid$outcome>0,] ## deleting na values
...
temp1 <- mid
temp1$cens <- ifelse(temp1$outcome==3, 1, 0)
temp2 <- mid; names(temp2)[1:2] <- c("ccode2","ccode1"); temp2 <- temp2[,names(mid)]
temp2$cens <- ifelse(temp1$outcome==4, 1, 0)
...
## save the data
save(durB,durG, file="C:/Users/YULENG/OneDrive/Documents/R Programming/DurationPaper/DurationTimeCov_v2.1.RData")
